...
const AchievementBadges = () => {
  ...
  const availableBadges = [
    ...
    },
    {
      ...
      description: "Helped peers by posting in the forum",
      icon: "🤝",
      category: "Community",
      requirement: "Make 5 forum posts"
    },
    {
      id: "wellness_warrior",
  ...
  const checkAndAwardBadges = async () => {
    ...
    if (!earnedBadgeIds.includes("community_helper") && userStats.forumPosts >= 5) {
      newBadges.push({
        user_id: user.id,
    ...
    if (newBadges.length > 0) {
      await supabase.from('user_achievements').insert(newBadges);
  ...
  const getProgressForBadge = (badge: any) => {
    switch (badge.id) {
      ...
      case "community_helper":
        return Math.min((userStats.forumPosts / 5) * 100, 100);
      default:
        return 0;
    }
  };
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="p-4 bg-orange-50 rounded-lg text-center">
              ...
              <h4 className="font-medium text-orange-900">Community</h4>
              <p className="text-sm text-orange-700">Help and support others</p>
            </div>
          </div>
        </CardContent>
      </Card>
